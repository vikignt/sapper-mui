<DemoPanel>
	<div class="container">
		...
		<ExpansionPanel
			name="Panel 2"
			bind:group={group[accordion ? 0 : 1]}
			{dense}
			on:change={onchage}
		>
			<div>
		...
		<ExpansionPanel
			name="Panel 3"
			bind:group={group[accordion ? 0 : 2]}
			{dense}
			rotate={false}
			on:change={onchage}
		>
		...
		<ExpansionPanel
			name="Panel 4"
			bind:group={group[accordion ? 0 : 3]}
			{dense}
			on:change={onchage}
		>
			<div slot="icon">
	...
	<div slot="action">
		<Checkbox bind:checked={accordion}>accordion</Checkbox>
...
<script>
	import { ExpansionPanel, Checkbox, Icon } from 'svelte-mui/src';
