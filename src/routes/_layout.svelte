...
<svelte:head>
	<title>sapper-mui{titlePage ? `: ${titlePage}` : ''}</title>
</svelte:head>
...
<LeftPanel bind:visible={leftPanelVisible} currentPath={$page.path} {sitenav} />
...
<main>
	{#if maxWidth > 720}
		<div class="nav-panel" transition:fly={{ x: -224, duration: 150, easing: linear }}>
			<Nav routes={sitenav} currentPath={$page.path} />
		</div>
	{/if}
	...
	<div class="page">
		...
		{#if maxWidth < 721 && segment === undefined}
			<div
				class="explore"
				tabindex="0"
				on:click={() => {
...
<script>
	...
	$: {
		const route = sitenav.find((item) => item.path === $page.path);
		titlePage = route ? route.name : '';
	}
